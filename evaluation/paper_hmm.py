"""Paper-aligned supervised Gaussian HMM (Wang & Hirsa).
...
def _ewm_volatility(returns: np.ndarray, span: int) -> np.ndarray:
    """Paper ground-truth feature: exponentially weighted moving volatility."""
    volatility = (
...
def _detect_change_points(
    ...
    """Binary segmentation approximation to the paper's change-point step."""
...
def identify_regimes_paper(
    ...
    """Paper-aligned five-regime ground truth.

    The paper computes EWM volatility, detects change points, and clusters the
    ...
    diagnostics = {
        "method": "paper_ewmv_changepoint_spectral",
...
def assign_regimes_by_centers(
    ...
    """Assign new observations to the nearest paper-regime volatility center."""
...
class PaperSupervisedGaussianHMM:
    ...
    transitions use the posterior mean under the paper's Dirichlet(1/K) prior,
    ...
    def fit(self, observed_returns: np.ndarray, regime_labels: np.ndarray):
        ...
        # The paper initializes the hidden sequence in Regime 0.
...
def fit_paper_supervised_hmm(
    observed_returns: np.ndarray,
    regime_labels: np.ndarray,
    n_regimes: int = 5,
) -> PaperSupervisedGaussianHMM:
    """Fit the paper's supervised state-conditioned Gaussian HMM."""
    return PaperSupervisedGaussianHMM(n_components=n_regimes).fit(
